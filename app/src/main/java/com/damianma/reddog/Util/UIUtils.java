...
/**
 * Author: ZT on 2017/1/12.
 */
public class UIUtils {
    public static Context getContext(){
        return RedDogApplication.getInstance();
    }
    public static Handler getHandler(){
        return RedDogApplication.getHandler();
    }
    // 根据布局id返回一个view布局
    ...
    public static boolean isRunMainThread(){
        ...
        if(currentThread==RedDogApplication.getMainThreadId()){
            return true;
        }
        return false;
    }
    ...
    */
    ...
    public static void showErrorToast(Context context, int responseCode){
        if(responseCode==404){
            ComUtils.showShortToast("未找到服务器(404)");
        }else if(responseCode==500){
            ComUtils.showShortToast("服务器内部错误(500)");
        }else {
            if(!NetworkUtil.checkedNetWork(context)){
                ComUtils.showShortToast("网络异常,请连接成功后再试");
            }else {
                ComUtils.showShortToast("操作失败，请重试");
            }
        }
    }
