...
import com.damianma.reddog.Util.SSLUtil;
import com.damianma.reddog.Util.StringUtil;
...
public class HttpManager {
    ...
    public static HttpManager getInstance() {
        if (instance == null) {
            synchronized (HttpManager.class) {
                if (instance == null) {
                    instance = new HttpManager(RedDogApplication.getInstance());
                }
            }
        }
        return instance;
    ...
    public static HttpManager setTokenValue(String tokenValue) {
        HttpManager.tokenValue = tokenValue;
        return instance;
    }
    ...
    public void get(final List<Parameter> paramList, final String url,
        ...
        new AsyncTask<Void, Void, Exception>() {
            ...
            protected Exception doInBackground(Void... params) {
                ...
                try {
                    result = getResponseJson(client, new Request.Builder().addHeader(KEY_TOKEN, tokenValue)
                            .addHeader(KEY_TOKEN, tokenValue)
                            .addHeader("device-type", "android")
                            .addHeader("device-id", android.os.Build.SERIAL)
                            .addHeader("x-timestamp", String.valueOf(new Date().getTime() / 1000))
                            .addHeader("request_timestamp", String.valueOf(new Date().getTime() / 1000))
                            .addHeader("client-version", "1.0.0")
                            .addHeader("system-version", "1.0.0")
                            .url(sb.toString())
                            .build());
        ...
        new AsyncTask<Void, Void, Exception>() {
            ...
            protected Exception doInBackground(Void... params) {
                ...
                try {
                    // TODO: 2018/10/25
                    //记得删除
//                    tokenValue = "37a3d69a2c3d09326ca54bfe0ba7f6bb";
                    result = getResponseJson(client, new Request.Builder().addHeader(KEY_TOKEN, tokenValue)
//					.addHeader(KEY_TOKEN, tokenValue)
                            .addHeader("device-type", "android")
                            .addHeader("device-id", Build.SERIAL)
                            .addHeader("x-timestamp", String.valueOf(new Date().getTime() / 1000))
                            .addHeader("request_timestamp", String.valueOf(new Date().getTime() / 1000))
                            .addHeader("client-version", "1.0.0")
                            .addHeader("system-version", "1.0.0")
